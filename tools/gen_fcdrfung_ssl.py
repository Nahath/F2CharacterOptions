"""
Read RP source FCDRFUNG.SSL, replace the RP-specific include block with
data/headers/ compatible includes, apply companion-implant targeting
changes, and write to data/scripts/fcdrfung.ssl.
"""
from pathlib import Path

SRC = Path(r"C:\git\f2Mod\F2Modding\BIS mapper patched (+complementary ressources)\Updated source codes\RP 2.3.3 source codes by Killap\SANFRAN\FCDRFUNG.SSL")
DST = Path(r"C:\git\f2Mod\data\scripts\fcdrfung.ssl")

NEW_HEADER = """\
#define SCRIPT_REALNAME "fcdrfung"
#include "../headers/define.h"
#include "../headers/fcdrfung.h"
...
procedure NodeSelectTarget begin
   Reply(241);
   NOption(242, NodeSlotPlayer, 004);
...
procedure NodeFullyImplanted begin
   Reply(244);
   NOption(243, NodeSelectTarget, 004);
end
"""

lines = SRC.read_text(encoding="latin-1").splitlines(keepends=True)

# Find the sanfran.h include line — that's the last include in the header block.
start = None
for i, line in enumerate(lines):
    if line.strip().startswith('#include') and 'sanfran' in line.lower():
        start = i
        break

if start is None:
    raise RuntimeError("Could not find sanfran.h #include line")

body = "".join(lines[start + 1:])

# Insert target_critter variable declaration after Only_Once variable
body = body.replace(
...
# Redirect implant entry point from Node034 to NodeSelectTarget (appears in Node001 and Node022)
body = body.replace(
    "NOption(206, Node034, 4);",
    "NOption(206, NodeSelectTarget, 4);",
)
