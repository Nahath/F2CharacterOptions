...
- The item proto must be the **Misc Item** type and have the **Use** action flag (`ITEM_ACTION_USE = 0x800` in `ExtendedFlags`)
...
- __NOTE:__ the hook lives inside `obj_use_explosive`, but the engine calls `obj_use_radio` first. `obj_use_radio` checks the **object's in-memory `sid` field** — not the proto. If `sid != -1` (even if the script file is empty), `obj_use_radio` consumes the use event and `obj_use_explosive` is never reached, so `item_make_explosive` silently does nothing.
- __Map-placed objects:__ For map-placed objects, the SID is baked into the map file at placement time and is NOT re-read from the proto at load time. Setting the proto's `ScriptID` to -1 does NOT clear the SID for already-placed objects. Fix: in the `game_loaded` handler, iterate the player's inventory and call `set_object_data(item, OBJ_DATA_SID, -1)` (`OBJ_DATA_SID = 0x78` from `define_extra.h`) for any items with the affected pid. See `debugging-facts.md §item_make_explosive silently fails` for a complete example.
